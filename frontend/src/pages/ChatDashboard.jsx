...
function ConsultationCard({ consultation, onClick }) {
  ...
  return (
    <div 
      ...
      className="flex flex-col p-5 rounded-xl bg-white dark:bg-card-dark border border-neutral-200 dark:border-sidebar-border hover:border-primary/50 transition-colors group cursor-pointer shadow-sm"
    >
...
export default function ChatDashboard() {
  ...
  const fetchChats = async () => {
    try {
      ...
      setConsultations(response.chats || []);
    } catch (err) {
  ...
  const handleNewChat = async () => {
    try {
      ...
      // Navigate to chat page even if API fails
      navigate('/chat/new');
    }
  };
